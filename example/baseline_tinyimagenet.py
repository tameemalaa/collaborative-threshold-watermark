...
clients = [ClientResNetTinyImageNet(model, device, initial_lr, client_flip_vectors[i], c, beta) for i in range(K)]
...
# Results Directory
# -------------------------
results_dir = f"results_tinyimagenet_seed{seed}"
...
for client_idx, client in enumerate(clients):
    ...
    initial_cos_sim.append(mean_cos_sim)
    z_score = (mean_cos_sim - mean) / std
    initial_zscores.append(z_score)
client_zscore_history = [[initial_zscores[i]] for i in range(K)]
client_cosine_history = [[initial_cos_sim[i]] for i in range(K)]  # If you want to keep cosine for debugging
...
final_model_filename = f"{results_dir}/tinyimagenet_fedavg_baseline_K{K}_lr{initial_lr}_c{c}_steps{num_steps}_bs{batch_size_per_client}_seed{seed}.pt"
