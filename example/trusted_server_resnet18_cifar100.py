...
from src.data_utils import get_cifar100_transforms, get_cifar100_dataset, get_client_subsets, get_client_loaders, get_cifar100_dataloaders
import pickle
# -------------------------
# Hyperparameters
# -------------------------
...
num_steps = 350  # increased steps for CIFAR-100 complexity
log_interval = 1
plot_interval = 1
...
wandb.init(
    project="trusted_server_resnet18_cifar100",
    config={
        "K": K,
...
clients = [ClientResNet18(model, device, initial_lr, flip_vectors, c, beta) for _ in range(K)]
transform_train, transform_test = get_cifar100_transforms()
train_dataset, validation_dataset, test_dataset = get_cifar100_dataset(transform_train, transform_test)
...
train_loader, validation_loader, test_loader = get_cifar100_dataloaders(train_dataset, validation_dataset, test_dataset, batch_size=batch_size_per_client)

client_index = 0  # Change this to export a different client's loader

# Create directory for exported data
os.makedirs("exported_data_cifar_100", exist_ok=True)
...
with open(f"exported_data_cifar_100/k{K}_C{c}_cifar100_loader_data.pkl", "wb") as f:
    pickle.dump(client_export_data, f)

# Save the test export data
with open(f"exported_data_cifar_100/k{K}_cifar100_test_loader_data.pkl", "wb") as f:
...
# Results Directory
# -------------------------
results_dir = "results_cifar100_trusted_server"
...
final_model_filename = f"{results_dir}/cifar100_fedavg_watermark_K{K}_lr{initial_lr}_c{c}_steps{num_steps}_bs{batch_size_per_client}_seed{seed}.pt"
torch.save(model.state_dict(), final_model_filename)
print(f"Final model saved to {final_model_filename}")

flip_vectors_filename = f"{results_dir}/cifar100_flip_vectors_K{K}_lr{initial_lr}_c{c}_steps{num_steps}_bs{batch_size_per_client}_seed{seed}.pt"
...
torch.save(attack_key, f"cifar100_attack_key_K{K}_lr{initial_lr}_c{c}_steps{num_steps}_bs{batch_size_per_client}_seed{seed}.pt")
